...
/*
===================================================================
Plugin Name:  ONet Auto Headline Anchor
Plugin URI:   http://onetdev.com/repo/onet-regen-thumbnails
Description:  For advanced users! Github-like headline parser for your posts, pages and custom contents. Also lets you fetch TOC for any content.
Git:          https://bitbucket.org/orosznyet/auto-headline-anchor
Version:      1.02
...
add_action( 'init', 'ONetAutoHeadlineAnchor_wrap' );
function ONetAutoHeadlineAnchor_wrap() {
global $ONetAutoHeadlineAnchor_inst;
	$ONetAutoHeadlineAnchor_inst = new ONetAutoHeadlineAnchor();
}

// Installation stuff
register_activation_hook(__FILE__,'ONetAutoHeadlineAnchor_install');
function ONetAutoHeadlineAnchor_install () {
	...
	if ( version_compare("5.0", phpversion(), '>') ) {
		echo '<strong>'.__('Plugin installation failed.','onetaha').'</strong> '.
			sprintf(__('Sorry, ONet Auto Headline Anchor requires at least PHP 5.0 but your version is PHP %s.','onetaha'),phpversion());
		exit;
	}
...
class ONetAutoHeadlineAnchor {
	...
	**/
	public function __construct() {
		...
		load_plugin_textdomain('onetaha', false, '/onet-auto-headline-anchor/lang');

		# Load options
		$default_opts = array(
			"enable" =>       true,                   // Global variable
			"link_style" =>   "git",                  // Headlink linkify method
			"add_link" =>     false,                  // Add a link element to a particular headline
	...
	**/
	function register_client_files () {
		...
		wp_register_style( 'onet-auto-headline-git', plugins_url($this->plugin."/assets/css/git.css") );

		if ($this->opts->smoothy) wp_enqueue_script('onet-smoothy');
		if ($this->opts->link_style == "git" && $this->opts->load_css) wp_enqueue_style('onet-auto-headline-git');
	}
	...
	**/
	public function get_settings_meta () {
		...
		# Add self links at the begining of each headline
		$fields['link_style'] = (object)array(
			...
			"html_output" =>
				...
				'<input type="radio" name="'.$this->prefix.'link_style" id="'.$this->prefix.'link_style_convert" value="convert" '.($this->opts->link_style == "convert" ? ' checked="checked"' : '').'/> '.__("Convert headlines into links",'onetaha').
	...
	**/ 
	...
	global $wpdb,$wp_settings_sections, $wp_settings_fields;
		...
		add_settings_section($this->prefix.'settings', __("ONet Auto Headline Anchor settings","onetaha"), null, 'reading' );
	...
	**/
	public function perform_parse ($content,$force=false) {
		...
		if (!$force && $cached) {
			$from_cache = $this->load_cache($cached)."<!-- ".__("ONet Auto Headline Anchor: Loaded from inner cache","onetaha")." -->";
		...
		return $content."<!-- ".__("ONet Auto Headline Anchor: Recently parsed","onetaha")." -->";
	}
	...
	**/
	function get_toc ($content, $preserved=true) {
		...
		for ($i=0;$i<count($matches[0]);$i++) {
			...
			# Generate update class for head
