...
Tags: headline, anchor, auto, post, toc
...
1. Install ONet Auto Headline Anchor either via the WordPress.org plugin directory, or by uploading the files to your server
...
In order to avoid multiple instances of the same link name the plugin works only on single pages. (You can still call it manually using ONetAutoHeadlineAnchor::perform_parse([content]) )

= How can I get TOC of a content? =

It is simplitcity itself. Use ONetAutoHeadlineAnchor::get_toc([content],[optional: return hierarchical or raw list]).
...
= 1.02 =
Missing settings link issue fix
